...
function ui:add_condition(condition)
    if type(condition) ~= "function" then
        print("[NN-UI] Condition must be a function!")
        return self
    end
...
function ui:set_callback(callback)
    if type(callback) ~= "function" then
        print("[NN-UI] An error occured while setting callback for element: " .. self.uid .. " - Expected function, got " .. type(callback))
        return self
    end
...
function ui:set(value)
    ...
    if not success then
        print("[NN-UI] An error occured while setting value for element: " .. tostring(self.uid) .. " - " .. err)
    end
end
...
function ui:init()
    ...
    if not pcall(function() return g_ui.get(self.parent.ref) end) then
        print("[NN-UI] An error occured while creating element: " .. uid .. " - Invalid parent element type!")
        return
    end
...
local trigger_speed = ui.new_slider("poo", "lua", "b", "Timer", 0, 100, 10) 
local key = ui.new_hotkey("lua", "b", "bind")
local function exploit(cmd)
    local me = entity.get_local_player()
    if me == nil then return end
    local timer = entity.get_prop(me, "m_nTickbase") % math.random(4, 8) == 0
    local air = bit.band (entity.get_prop(me, "m_fFlags"), 1) == 0
    if air then
        ui.set(ref_duckpeek, timer and "Always on" or "Toggle")
        ui.set(ref_aim_check, ui.get(key) and true or false)
...
paradox.menu.info.discord:set_callback(function()
    clipboard.set("https://discord.gg/og4leaks")
...
client.color_log(204, 135, 255, "              &5!Y   Our Discord: discord.gg/og4leaks")
client.color_log(204, 135, 255, "                &#")
client.color_log(204, 135, 255, " ")
client.color_log(204, 135, 255, " ")
client.color_log(204, 135, 255, " ")
